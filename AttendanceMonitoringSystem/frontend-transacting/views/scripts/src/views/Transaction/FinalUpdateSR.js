...
var FinalUpdateCAS = React.createClass({
    ...
    submitRefNo: function (ref_no) {
        if (ref_no) {
            this.context.router.push('/main/transaction/deal_' + this.state.category + '/' + this.state.transaction_type + '/final_update_sr/' + ref_no);
        }

        this.setState({show_ref_no_modal: false});
    },
    onClickRow: function (row_id) {
        this.context.router.push('/main/transaction/deal_' + this.state.category + '/' + this.state.transaction_type + '/final_update_sr/' + row_id);
    },
    render: function () {
        return (
            <TransactionContainer title={this.state.category.toUpperCase() + ' ' + this.state.transaction_type.toUpperCase() + ' FINAL UPDATE on Special Request'}>
                {
                    this.state.show_ref_no_modal ?
                        <RefNoInputModal submitRefNo={this.submitRefNo} title={this.state.category.toUpperCase() + ' ' + this.state.transaction_type.toUpperCase() + " FINAL UPDATE on Special Request"} />
                        : ''
                }

...
public class MFReceiveDetailsUtil{
	...
	public static List<HashMap<String, String>> receiveDetails(String messageDetails, String transactionCode,
		...
		int gmaxdlen = AppConstants.FX_MAXDLEN;
...
////	        map = (LinkedHashMap<String, TableFieldProperty>) m.invoke(null, null);
		...
		switch(transactionCode){
			...
			default:
				occursSw = false;
				occurLength = 0;
				String strippeMessage = StringUtils.stripEnd(messageDetails, StringUtils.SPACE);

				w_length = computeMessageLength(map);
				occurLength = w_length;
				
				mRecordCount = StringUtils.length(strippeMessage) / (double) w_length;
				
				if((mRecordCount - (int) mRecordCount) > 0){
					mRecordCount += 1;
				}
				
				records = parseMessage(strippeMessage, (int) mRecordCount, map);
				return records;
		}
		...
		if(occursSw){
			...
			headLength = computeMessageLength(map, 0, fieldNumOne - 1);

			if(fieldNumA == 0){
				occurLength = computeMessageLength(map, fieldNumOne - 1, fieldNumTwo);
				int totalLength = headLength + (occurLength * occurTimes);

				rMessageDetails = StringUtils.stripEnd(messageDetails, StringUtils.SPACE);
				messageDetails = StringUtils.left(rMessageDetails, totalLength);

				while(!StringUtils.isBlank(rMessageDetails)){
					xMessageDetails = StringUtils.stripEnd(messageDetails, StringUtils.SPACE);
					mRecordCount = xMessageDetails.length() - headLength;
					mRecordCount = mRecordCount / (double) occurLength;
					if((mRecordCount - (int) mRecordCount) > 0){
						mRecordCount += 1;
					}
					if("0327".equals(transactionCode)){
					}

					List<HashMap<String, String>> newRecords = parseSpecialMessage(messageDetails, (int) mRecordCount,
							fieldNumOne - 1, fieldNumTwo, headLength, occurType, totalLength, map);
				...
				int totalLength = 0;
				occurLength = computeMessageLength(map, fieldNumOne - 1, fieldNumA);
			...
			for(int i = fieldStart; (i < fieldEnd) && (i < propertyEntries.size()); i++){
				...
				switch(property.getDataType()){
					...
					case 'N':
						...
						length = value.length();
						value = (property.getDecimal() == 0) ? value
								: new StringBuilder(value).insert(value.length() - property.getDecimal(), ".").toString();
						break;
		...
		if((map != null) && !map.isEmpty()){
			...
			for(int i = start; i < end; i++){
				...
				switch(property.getDataType()){
					...
					case 'D':
						messageLength += property.getIsDate8Format()? property.getLength() : property.getLength() - 2;
						break;
